...
class GameActionCounter(SimulationAccumulator):
    # Counter variables for tracking and saving
    robber_counter = 0
    trade_counter = 0
    dev_counter = 0
    knight_counter = 0
    ...
    # Variables for tracking game and turn counts for saving
    game_counter = 75
    turn_count = 0
    
    # Variaables for tracking buildings for saving
    cities = 0
    settlements = 0
    roads = 0
    
    # Variables for tracking victory point cards and victory points for saving
    vpcards = 0
    vp = 0
    
    # To allow the accumulator to perform state fucntions
    color = Color("WHITE")
    # csv = 'test1_lite.csv'
    csv = 'video.csv'# Path for saving and accessing CSV file
    
    def before(self, game):
        # Count updates and resets
        ...
        if not os.path.exists(self.csv): # Creates file if not made yet, initlisaes header row
            header = [
        ...
        with open(self.csv, mode='r') as file:
            ...
            if max_game_counter_row:
                self.game_counter = int(max_game_counter_row['game']) # If there is pre-existing data it will continue adding on
        
        with open(self.csv, mode='a', newline='') as file: # Creates the first row if theres no preexisting data
            self.game_counter += 1
            writer = csv.writer(file)
            row = [self.game_counter] + [0] * 14  # Initialize new row for a new game with zeroes
            writer.writerow(row)
        
    def step(self, game_after_action, action):
        # Update variables during each action taken by the model
        ...
        # Updates victory point trackers
    ...
    def after(self, game):
        # Due to the game clearing the data after it ends, we check the wining color
        # and if it is the model then we override its last victory point count with 10 (win condition)
        ...
        # Find the row with the largest game_counter (current row with mostly zeroes)
        max_game_counter_row = max(rows, key=lambda x: int(x['game']), default=None)
        if max_game_counter_row:
...
def create_q_network(state_shape, action_space):
    ...
    # First Hidden Layer of 64 Neurons
    model.add(Dense(64, activation='relu'))
    model.add(Dropout(0.2)) # Dropout layer
    
    # Second Hidden Layer of 4 Neurons
...
class RLModel(Player):
    ...
    def save_model(self):
        ...
        model_path = os.path.join(os.path.dirname(__file__), "rl_test_lite.h5")
